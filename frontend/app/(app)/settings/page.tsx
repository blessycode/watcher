...
export default function SettingsPage() {
  ...
  return (
    <div className="space-y-4 text-[#F3F4F6]">
      ...
      <SettingsCard title="API Keys" description="Manage tokens for the Watcher API." actions={<Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><button className="inline-flex h-7 items-center gap-1 rounded bg-[#4F8CFF] px-3 text-[11px] text-white"><Plus className="h-3 w-3" /> Create key</button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Create API key</DialogTitle></DialogHeader><div className="space-y-3"><div className="space-y-1.5"><Label>Name</Label><Input value={name} onChange={(event) => setName(event.target.value)} placeholder="Production worker" /></div>{newKey && <div className="rounded border border-emerald-500/30 bg-emerald-500/10 p-3"><div className="text-xs font-semibold text-emerald-300">Copy this key now. It will not be shown again.</div><code className="mt-2 block break-all text-xs">{newKey}</code></div>}<div className="flex justify-end gap-2"><Button variant="outline" onClick={() => setOpen(false)}>Close</Button><Button onClick={createKey} disabled={!name}>Create</Button></div></div></DialogContent></Dialog>}>
