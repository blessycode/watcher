      ...
      <div className="rounded-xl border border-border bg-card p-4">
        ...
        <div className="overflow-x-auto font-mono text-xs">
          <table className="w-full">
            ...
            <tbody>{checks.map((check) => <tr key={check.id} className="border-b border-border/60"><td className="px-2 py-1.5 text-muted-foreground">{new Date(check.checked_at).toLocaleString()}</td><td className="px-2 py-1.5 text-muted-foreground">{check.region}</td><td className={`px-2 py-1.5 ${check.success ? "text-emerald-500" : "text-red-500"}`}>{check.status_code ?? "-"}</td><td className="px-2 py-1.5 text-right">{check.latency_ms ? `${check.latency_ms}ms` : "-"}</td><td className="px-2 py-1.5 text-muted-foreground">{check.error_message || "OK"}</td></tr>)}</tbody>
          </table>
